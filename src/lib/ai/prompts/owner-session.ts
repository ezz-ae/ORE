...
export function buildOwnerSessionPrompt(input: OwnerSessionRequest) {
  return `Return only valid JSON for a paid ${BRAND.company} owner session with keys: propertyAngle, targetAudience, metaAdCopy, instagramCaption, whatsappScript, leadForm, followUpScript, launchChecklist, mistakesToAvoid.
