    ...
    <div className="mx-auto max-w-4xl px-4 pb-32 pt-10 sm:px-6 sm:pt-14">
      ...
      <section className="mt-7">
        ...
        <h1 className="mt-4 text-[36px] font-semibold leading-[1.05] tracking-tight text-white sm:text-[48px]">
          WhatsApp Business<br /><span className="text-white/35">blocked by {critical} item{critical !== 1 ? 's' : ''}.</span>
        </h1>
      ...
      {/* Automation flows */}
      <section className="mt-14">
        <div className="text-[11px] font-medium uppercase tracking-[0.22em] text-white/40">Automations</div>
        <h2 className="mt-2 text-xl font-semibold text-white">Flows enabled once connected</h2>
        <div className="mt-5 space-y-3">
          {AUTOMATION_FLOWS.map((flow) => (
            <div key={flow.id} className="rounded-[18px] border border-white/[0.06] bg-[#0A0D10] p-5">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={`shrink-0 text-[11px] font-medium ${flow.statusColor}`}>{flow.status}</span>
              </div>
              <div className="mt-3 rounded-[10px] border border-white/[0.04] bg-white/[0.02] px-4 py-3">
                <p className="font-mono text-[11px] leading-relaxed text-white/40">{flow.template}</p>
              </div>
            </div>
          ))}
        </div>
