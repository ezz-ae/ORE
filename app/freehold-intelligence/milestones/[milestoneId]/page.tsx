    ...
    <div className="mx-auto max-w-3xl px-6 pb-16 pt-6 sm:pt-16">
      ...
      <section className="mt-7">
        ...
        <p className="mt-7 max-w-2xl text-lg leading-[1.6] text-slate-300">
          {milestone.description || t('pmile.descriptionFallback')}
        </p>
      </section>

      <section className="mt-12 grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-line bg-surface p-5">
          <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">{t('pmile.owner')}</div>
          <div className="mt-2 text-lg font-semibold tracking-tight text-white">{milestone.owner}</div>
        </div>
        <div className="rounded-2xl border border-line bg-surface p-5">
          <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">{t('pmile.deadline')}</div>
          <div className="mt-2 text-lg font-semibold tracking-tight text-white">{milestone.deadline}</div>
          {milestone.days_to_deadline != null && (
            <div className="mt-1 text-xs text-slate-400">{t('pmile.daysRemaining', { n: milestone.days_to_deadline })}</div>
          )}
        </div>
        <div className="rounded-2xl border border-line bg-surface p-5">
          <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">{t('pmile.health')}</div>
          <div className={`mt-2 flex items-center gap-1.5 text-lg font-semibold capitalize ${tone.text}`}>
            <span className={`h-2 w-2 rounded-full ${tone.dot}`} />
            {t(`pmile.healthValue.${milestone.health ?? 'planned'}`)}
          </div>
        </div>
      </section>
      ...
      {milestone.success_event && (
        <section className="mt-12 rounded-3xl border border-gold/15 bg-gold/[0.04] px-7 py-7 sm:px-10 sm:py-9">
          <div className="text-xs font-semibold uppercase tracking-wider text-gold/85">{t('pmile.successEvent')}</div>
          <p className="mt-3 text-base font-medium leading-[1.65] text-slate-100 sm:text-lg">{milestone.success_event}</p>
        </section>
      )}

      <section className="mt-16">
        <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">{t('pmile.commentsEyebrow')}</div>
        <h2 className="mt-2 mb-6 text-2xl font-semibold tracking-tight text-white">{t('pmile.reviewTitle')}</h2>
