...
function CopyButton({ text, className }: { text: string; className?: string }) {
  ...
  return (
    <button
      ...
      className={
        className ??
        'inline-flex items-center gap-1 text-sm text-slate-600 transition hover:text-slate-400'
      }
    >
      {copied
...
function CharBadge({ text, limit, warn }: { text: string; limit: number; warn: number }) {
  ...
  if (len > limit) {
    return (
      <span className="ml-1.5 rounded px-1.5 py-0.5 text-xs font-medium bg-red-400/[0.12] text-red-400">
        {len}/{limit}
      </span>
    )
  }
  if (len > warn) {
    return (
      <span className="ml-1.5 rounded px-1.5 py-0.5 text-xs font-medium bg-orange-400/[0.12] text-orange-300">
        {len}/{limit}
      </span>
    )
  }
  return (
    <span className="ml-1.5 rounded px-1.5 py-0.5 text-xs text-slate-600">
      {len}/{limit}
    </span>
  )
}
    ...
    <div className="mx-auto max-w-5xl px-4 pb-16 pt-6 sm:px-6 sm:pt-8">
      ...
      <section className="mt-7">
        ...
        <h1 className="mt-4 text-2xl font-semibold tracking-tight text-white">
          ...
          <span className="text-slate-500">headlines + descriptions.</span>
        </h1>
      </section>
            ...
            <div className="flex min-h-[400px] flex-col items-center justify-center rounded-[22px] border border-dashed border-line px-8 text-center">
              ...
              <div className="mt-4 max-w-xs rounded-[14px] border border-line bg-surface-2 px-4 py-4 text-left">
                ...
                <div className="space-y-1.5 text-sm text-slate-500">
                  <div>
                    <span className="text-slate-400 font-medium">Headlines</span> — up to 15, max 30 chars each
                  </div>
                  <div>
                    <span className="text-slate-400 font-medium">Descriptions</span> — up to 4, max 90 chars each
                  </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mb-4 flex items-start justify-between gap-3">
                      <div>
                        <span className="rounded-full bg-[#4285F4]/15 border border-[#4285F4]/25 px-2.5 py-1 text-sm font-semibold text-[#4285F4]">
                          Variant {i + 1}
                        </span>
                        {v.note && (
                          <p className="mt-2 text-sm leading-relaxed text-slate-500">{v.note}</p>
                        )}
                      </div>
                      <CopyButton
                        text={copyAllText}
                        ...
                        {v.descriptions.map((desc, dIdx) => (
                          ...
                          >
                            <div className="flex items-baseline gap-2 min-w-0">
                              ...
                              <span className="text-xs text-slate-300 leading-snug break-words">
                                {desc}
                              </span>
                              <CharBadge
                                text={desc}
                    ...
                    {warnings.length > 0 && (
                      <div className="mt-3 rounded-[10px] border border-red-400/15 bg-red-400/[0.04] px-3 py-2.5">
                        <div className="mb-1 flex items-center gap-1.5 text-xs font-medium uppercase tracking-[0.14em] text-red-400/70">
                          <AlertCircle className="h-3 w-3" /> Character limit exceeded
                        </div>
                        <div className="space-y-0.5">
                          {warnings.map((w, wIdx) => (
                            <div key={wIdx} className="text-sm text-red-400/70">
      ...
      <div className="mt-12 rounded-[20px] border border-[#4285F4]/15 bg-[#4285F4]/[0.04] p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <div className="mb-1.5 text-sm font-semibold uppercase tracking-[0.18em] text-[#4285F4]/70">
              RSA Best Practices
            </div>
