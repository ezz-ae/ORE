    ...
    <div className="mx-auto max-w-4xl px-4 pb-32 pt-10 sm:px-6 sm:pt-14">
      ...
      <section>
        ...
        <p className="mt-5 max-w-xl text-[16px] leading-[1.65] text-white/60">
          ...
          {stats.resolved > 0 && (
            <span className="ml-2 text-emerald-400/80">{stats.resolved} resolved this session.</span>
          )}
        </p>
      </section>
      ...
      <section className="mt-8 grid grid-cols-4 gap-3">
        ...
        <div className="rounded-[18px] border border-emerald-400/20 bg-emerald-400/[0.06] p-4 text-center">
          <p className="text-[26px] font-semibold text-emerald-300">{stats.approvals}</p>
          <p className="text-[10px] text-emerald-400/60 mt-1">Approvals</p>
        </div>
        ...
        <div className="rounded-[18px] border border-emerald-400/15 bg-emerald-400/[0.04] p-4 text-center">
          <p className="text-[26px] font-semibold text-emerald-400/70">{stats.resolved}</p>
          <p className="text-[10px] text-emerald-400/40 mt-1">Resolved</p>
        </div>
      </section>

      {/* Type filter pills */}
      <div className="mt-6 flex flex-wrap items-center gap-2">
        <span className="text-[11px] text-white/35 shrink-0">Filter:</span>
        {TYPE_FILTERS.map((f) => (
          <button
            key={f}
        ...
        {hasFilter && (
          <button
            ...
            className="flex items-center gap-1 rounded-full border border-white/[0.08] bg-white/[0.03] px-2.5 py-0.5 text-[11px] text-white/40 transition hover:text-white/70"
          >
      ...
      <p className="mt-2 text-[11px] text-white/25">{filtered.length} of {stats.total} items shown</p>
      ...
      <section className="mt-8 rounded-[22px] border border-white/[0.06] bg-[#0A0D10] p-6">
        ...
        {commentFlash && (
          <div className="mb-4 flex items-center gap-2 rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-2.5 text-sm text-emerald-400">
