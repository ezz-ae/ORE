import { CheckSquare, AlertCircle, Clock, MessageSquare, CheckCircle2, ArrowUpRight, Sparkles } from 'lucide-react'
import Link from 'next/link'
import { AiPrompt } from '@/components/freehold/ai-prompt'

type ReviewItem = {
  id: string
  type: 'approval' | 'decision' | 'correction' | 'access request' | 'comment'
...
const reviewItems: ReviewItem[] = [
  ...
  },
  {
    ...
    project: 'Palm Jumeirah Investor Pack',
    projectHref: '/freehold-intelligence/lead-machine/listings/lm_palm_001',
    title: 'Hero CTA — investor comparison vs. generic availability',
    body: 'The current landing hero CTA reads "Check availability". For investor-intent traffic this should ask for a payment plan comparison. Needs copy update before approval.',
    owner: 'Marketing',
    requestedBy: 'Marketing',
    dueDate: '23 May',
    linkedTo: 'Landing · Palm copy',
  },
  {
    id: 'rv_005',
    type: 'decision',
    priority: 'medium',
    project: 'Business Bay Entry Offer',
    projectHref: '/freehold-intelligence/lead-machine/listings/lm_bay_003',
    title: 'Payment plan — add or proceed without?',
    body: 'Business Bay listing has no payment plan on record. A landing can technically be generated without it, but conversion rate will be lower. Decision needed: add Binghatti payment plan now or generate landing with price-only layout and update later.',
    owner: 'Data Manager',
    requestedBy: 'Marketing',
    dueDate: '24 May',
  ...
  },
  {
    ...
    body: 'The 70/30 construction-linked payment plan is a strong conversion signal. Adding a one-line callout above the lead form (e.g. "70/30 payment plan — AED 550,000 to start") could increase lead quality from paid traffic.',
    owner: 'Marketing',
    requestedBy: 'Marketing',
...
function typeTone(t: string) {
  if (t === 'approval')       return 'bg-emerald-400/10 border-emerald-400/20 text-emerald-200'
...
const typeIcon = (t: string) => {
  if (t === 'approval' || t === 'decision') return <CheckCircle2 className="h-3.5 w-3.5 text-emerald-400" />
  if (t === 'access request') return <AlertCircle className="h-3.5 w-3.5 text-red-400" />
  if (t === 'correction') return <MessageSquare className="h-3.5 w-3.5 text-sky-400" />
  return <Clock className="h-3.5 w-3.5 text-white/30" />
}

const critical = reviewItems.filter(r => r.priority === 'critical')
const approvals = reviewItems.filter(r => r.type === 'approval')
const totalOpen = reviewItems.length
    ...
    <div className="mx-auto max-w-4xl px-4 pb-32 pt-10 sm:px-6 sm:pt-14">
      ...
      <section>
        ...
        <h1 className="mt-4 text-[36px] font-semibold leading-[1.05] tracking-tight text-white sm:text-[48px]">
          {totalOpen} decisions waiting.
          <br />
          <span className="text-white/35">{critical.length} are launch blockers.</span>
        </h1>
        <p className="mt-5 max-w-xl text-[16px] leading-[1.65] text-white/60">
          Comments, approvals, corrections, and access requests — sorted by what blocks launch first. Resolve in priority order.
        </p>
      </section>
      ...
      <section className="mt-8 grid grid-cols-3 gap-3">
        <div className="rounded-[18px] border border-red-400/20 bg-red-400/[0.06] p-4 text-center">
          <p className="text-[26px] font-semibold text-red-300">{critical.length}</p>
          <p className="text-[10px] text-red-400/60 mt-1">Blockers</p>
        </div>
        <div className="rounded-[18px] border border-emerald-400/20 bg-emerald-400/[0.06] p-4 text-center">
          <p className="text-[26px] font-semibold text-emerald-300">{approvals.length}</p>
          <p className="text-[10px] text-emerald-400/60 mt-1">Approvals needed</p>
        </div>
            ...
            <div key={item.id} className={`rounded-[22px] border p-6 ${tone.ring} ${tone.bg}`}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-[11px] font-medium capitalize ${typeTone(item.type)}`}>
                    {typeIcon(item.type)}
                    {item.type}
                  </span>
                </div>
              ...
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-white/[0.05] pt-4">
                ...
                <div className={`text-[12px] font-medium ${tone.text}`}>
      ...
      <section className="mt-8 rounded-[22px] border border-white/[0.06] bg-[#0A0D10] p-6">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="inline-flex items-center gap-1.5 rounded-[12px] border border-[#D4AF37]/20 bg-[#D4AF37]/[0.06] px-4 py-2.5 text-[13px] font-medium text-[#F8E7AE] transition hover:border-[#D4AF37]/35 hover:bg-[#D4AF37]/10">
              + Add as task
            </button>
          </div>
        </div>
