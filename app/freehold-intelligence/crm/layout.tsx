      ...
      <header className="sticky top-0 z-40 flex h-14 shrink-0 items-center gap-4 border-b border-white/[0.07] bg-chrome/97 px-5 backdrop-blur-xl sm:px-6">
        ...
        <div className="ml-auto flex items-center gap-2">
          <Link
            ...
            className="flex items-center gap-1.5 rounded-lg border border-gold/25 bg-gold/[0.08] px-3 py-1.5 text-sm font-medium text-gold transition-colors hover:bg-gold/15"
          >
            ...
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  {tab.label}
                  {tab.label === 'Agents' && (
                    <span className="ml-auto text-xs font-semibold text-gold">3</span>
                  )}
                </Link>
              )
            })}
