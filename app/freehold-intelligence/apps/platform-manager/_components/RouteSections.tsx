  ...
  return (
    <div className="flex items-center gap-4 px-5 py-3.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 truncate text-[11px] text-white/35">{route.note}</p>
      </div>
...
export function RouteSections({ routes }: { routes: RouteEntry[] }) {
  ...
  return (
    <>
        ...
        {STATUS_PILLS.map(({ key, label }) => (
          <button
            ...
            className={[
              ...
              statusFilter === key
                ? key === 'live'    ? 'border-emerald-400/40 bg-emerald-400/10 text-emerald-300'
      ...
      {filtered.length === 0 && (
        <div className="mt-10 rounded-[22px] border border-white/[0.06] bg-white/[0.02] px-6 py-14 text-center text-[13px] text-white/30">
