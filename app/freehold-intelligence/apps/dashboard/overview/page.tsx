    ...
    <div className="mx-auto max-w-5xl px-4 pb-16 pt-6 sm:px-6 sm:pt-8">
      ...
      <section className="mt-12 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        {[
          ...
          <div key={kpi.label} className="rounded-[18px] border border-slate-800 bg-slate-900 p-4 text-center">
      ...
      <section className="mt-14">
        ...
        <div className="mt-5 grid gap-3">
          {INFRA.map((item) => {
            ...
            return (
              <div key={item.label} className="flex items-center justify-between gap-5 rounded-2xl border border-slate-800 bg-slate-900 p-5">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-800">
              {serverApps.filter((a) => a.status !== 'planned').slice(0, 8).map((app) => (
                <tr key={app.id} className="transition hover:bg-slate-800/30">
                  <td className="px-6 py-4">
                    <Link href={app.href} className="font-medium text-slate-100 transition hover:text-[#D4AF37]">{app.name}</Link>
                  </td>
                  ...
                  <td className="hidden px-4 py-4 text-center lg:table-cell">
                    <span className={`font-medium tabular-nums ${app.pendingApprovalCount > 0 ? 'text-[#D4AF37]' : 'text-slate-600'}`}>{app.pendingApprovalCount}</span>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <span className={`text-sm font-medium ${app.status === 'live' ? 'text-[#D4AF37]' : app.status === 'in_progress' ? 'text-[#F8E7AE]' : 'text-slate-500'}`}>
