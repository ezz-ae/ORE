...
const FUNNEL = [
  ...
  { stage: 'Converted',      value: 31,    prev: 26,    pct: 23.3, tone: 'bg-emerald-400' },
]

const SOURCE_BREAKDOWN = [
  { source: 'Meta Ads — investor',  leads: 92,  conv: 24, spend: 'AED 14,200', cpl: 'AED 154', color: 'bg-sky-400' },
  { source: 'WhatsApp inbound',     leads: 47,  conv: 28, spend: '—',          cpl: '—',       color: 'bg-emerald-400' },
    ...
    <div className="mx-auto max-w-5xl px-4 pb-32 pt-10 sm:px-6 sm:pt-14">
      ...
      <section className="mt-10 grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          ...
          { label: 'Conversion',  value: '23.3%',     delta: '+4.1pp', tone: 'text-emerald-300' },
          { label: 'Avg deal',    value: 'AED 2.4M',  delta: '+12%',  tone: 'text-emerald-300' },
          { label: 'Revenue MTD', value: 'AED 32M',   delta: '+16%',  tone: 'text-emerald-300' },
        ].map((kpi) => (
      ...
      <section className="mt-14">
        ...
        <div className="mt-5 space-y-2.5">
          {FUNNEL.map((stage, i) => {
            ...
            return (
              <div key={stage.stage} className="rounded-[18px] border border-white/[0.06] bg-[#0A0D10] p-5">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-4 min-w-0">
                    <span className="text-[11px] text-white/30 w-4 tabular-nums">{i + 1}</span>
                    <div className="min-w-0">
                      <div className="text-[13px] font-medium text-white/80">{stage.stage}</div>
                      {delta !== null && (
                        <div className={`text-[11px] ${delta >= 0 ? 'text-emerald-300' : 'text-red-300'}`}>
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="border-b border-white/[0.05]">
                <th className="px-6 py-3 text-left text-[10px] font-medium uppercase tracking-[0.18em] text-white/30">Source</th>
                <th className="px-4 py-3 text-right text-[10px] font-medium uppercase tracking-[0.18em] text-white/30">Leads</th>
                <th className="hidden px-4 py-3 text-right text-[10px] font-medium uppercase tracking-[0.18em] text-white/30 sm:table-cell">Conv %</th>
                <th className="hidden px-6 py-3 text-right text-[10px] font-medium uppercase tracking-[0.18em] text-white/30 md:table-cell">CPL</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/[0.05]">
              {SOURCE_BREAKDOWN.map((src) => (
                <tr key={src.source} className="transition hover:bg-white/[0.02]">
                  ...
                  <td className="hidden px-4 py-4 text-right text-emerald-300 sm:table-cell">{src.conv}%</td>
      ...
      <section className="mt-14">
        ...
        <div className="mt-5 rounded-[22px] border border-white/[0.06] bg-[#0A0D10] p-6 sm:p-8">
            ...
            {MONTHLY.map((m) => (
              <div key={m.month} className="flex flex-col items-center gap-3">
                ...
                <div className="text-center">
                  ...
                  <div className="text-[10px] uppercase tracking-[0.14em] text-white/35">{m.month}</div>
                  <div className="mt-0.5 text-[11px] text-white/40">AED {m.revenue}M</div>
                </div>
              </div>
            ))}
          </div>
