...
export default async function FreeholdIntelligenceHome() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 pb-32 pt-12 sm:pt-20">
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-24 space-y-4">
            ...
            <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] px-5 py-4">
              <div className="text-[10px] font-medium uppercase tracking-[0.22em] text-white/40">Open blockers</div>
              <div className="mt-2 flex items-center gap-3">
                <span className={`text-3xl font-semibold tabular-nums ${openBlockers > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
                  {openBlockers}
                </span>
                <span className={`h-2.5 w-2.5 rounded-full ${openBlockers > 0 ? 'bg-red-400 shadow-[0_0_10px_rgba(248,113,113,0.55)]' : 'bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.55)]'}`} />
              </div>
              <p className="mt-1 text-[12px] text-white/40">
                {openBlockers > 0 ? 'Action required' : 'All clear'}
              </p>
            </div>
            ...
            {totalProjects != null && (
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] px-5 py-4">
                <div className="text-[10px] font-medium uppercase tracking-[0.22em] text-white/40">Total projects</div>
                <div className="mt-2 text-3xl font-semibold tabular-nums text-white">
                  {Number(totalProjects).toLocaleString()}
                </div>
                <p className="mt-1 text-[12px] text-white/40">Across all portfolios</p>
              </div>
            )}
