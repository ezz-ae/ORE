...
export default function AgentProfilePage() {
  ...
  async function addLead() {
    ...
    const res = await fetch('/api/freehold/crm/leads', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: nlName.trim(), phone: nlPhone.trim() || undefined, assignedBrokerId: agentId, source: 'manual' }),
    }).catch(() => null)
    setAddBusy(false)
    if (res && res.ok) {
      ...
      <section className="rounded-2xl border border-line bg-surface-2 p-5">
        ...
        {addOpen && (
          <div className="mt-3 flex flex-wrap items-center gap-2 rounded-xl border border-line bg-surface p-3">
            <input value={nlName} onChange={(e) => setNlName(e.target.value)} placeholder={t('analytics.agent.act.namePh')}
