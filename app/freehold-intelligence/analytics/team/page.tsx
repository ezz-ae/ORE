...
type Agent = {
  ...
  totalLeads: number; hotLeads: number; overdueFollowUps: number
  utilization: number; status: string; recentWins: number
}
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-800">
                  {agents && agents.length > 0 ? (
                    agents.map((ag) => (
                      <tr key={ag.id} className="transition hover:bg-slate-800/40">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2.5">
                            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-slate-700 text-xs font-bold text-slate-200">{ag.initials}</span>
                        ...
                        <td className="px-4 py-3 text-right tabular-nums text-[#D4AF37]">{ag.recentWins}</td>
                        <td className="px-4 py-3 text-right tabular-nums text-slate-300">{ag.overdueFollowUps}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-2">
                            <div className="h-1.5 w-16 overflow-hidden rounded-full bg-slate-700">
                              <div className={`h-full rounded-full ${loadColor(ag.utilization)}`} style={{ width: `${Math.min(100, ag.utilization)}%` }} />
                            </div>
                            <span className="w-9 text-right text-xs tabular-nums text-slate-400">{ag.utilization}%</span>
                          </div>
                        </td>
                      </tr>
                    ))
