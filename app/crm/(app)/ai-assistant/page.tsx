...
const suggestions = [
  "Build an Freehold shortlist of Dubai Marina projects with 8%+ ROI",
  "Which leads should Freehold follow up with first today?",
  "Draft an Freehold follow-up email for a lead who viewed 3 properties",
  "Prepare an Freehold investment brief comparing Project A vs Project B",
  "Add a new listing called Freehold Marina Edge in Dubai Marina by Select Group starting from AED 1,850,000 with 8.4% ROI",
  "Create an Freehold-branded offer for a lead interested in Dubai Hills",
]
...
export default function DashboardAIAssistantPage() {
  ...
  const shareConversation = (title: string) => {
    const text = `Freehold AI Insight: ${title}`
  ...
  return (
    <div className="space-y-8 pb-16">
      <section className="rounded-3xl border border-border bg-gradient-to-br from-card to-muted/40 p-6 shadow-sm md:p-8">
        ...
        <h1 className="font-serif text-3xl font-bold tracking-tight md:text-4xl">Your Freehold AI Workspace</h1>
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
          Ask for Freehold follow-up ideas, curated shortlists, client messages in Freehold voice, or listing updates without leaving the CRM.
        </p>
      </section>
        ...
        <Card className="overflow-hidden rounded-[2rem] border-border bg-card/50 shadow-md backdrop-blur-sm">
          <CardHeader className="border-b border-border/50 bg-muted/20 p-5 pb-6 md:p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <CardTitle className="font-serif text-xl font-bold">Ask Freehold AI</CardTitle>
                <p className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground mt-0.5">
                  Freehold follow-up, stock support, and campaign help
                </p>
              </div>
            </div>
          ...
          <CardContent className="space-y-6 p-4 md:p-8">
            <div ref={scrollRef} className="max-h-[65dvh] space-y-4 overflow-y-auto overscroll-contain rounded-3xl border border-border/40 bg-card/40 p-4 shadow-inner md:max-h-[580px] md:p-6">
              {messages.length === 0 ? (
                <div className="rounded-xl border border-dashed border-border p-6 text-center text-sm text-muted-foreground">
                  Start with an Freehold buyer brief, listing task, or follow-up request.
                </div>
              ) : (
              ...
              {isLoading && <ChatMessage role="assistant" content="Preparing an Freehold-ready answer for you..." />}
            </div>
            {error && (
            ...
            <ChatInput
              ...
              disabled={isLoading}
              placeholder="Ask Freehold AI about buyers, listings, offers, or investment comparisons..."
            />
          </CardContent>
        </Card>
