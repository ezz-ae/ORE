...
type FigureAudit = { value: string; status: 'grounded' | 'derived' | 'ungrounded'; formula?: string }
type Message =
  | { role: 'user'; content: string; ref?: ExpertContextRef }
  | { role: 'assistant'; blocks: ExpertBlock[]; toolsUsed?: string[]; verified?: boolean; evidence?: FigureAudit[] }
...
export function ExpertChat() {
  ...
  const send = useCallback(async (text?: string, ref?: ExpertContextRef) => {
    ...
    try {
      ...
      const evidence: FigureAudit[] | undefined = Array.isArray(data?.data?.evidence) ? data.data.evidence : undefined
      setMessages((m) => [...m, { role: 'assistant', blocks, toolsUsed, verified, evidence }])
              ...
              {messages.map((m, i) =>
                m.role === 'user' ? (
                  ...
                  <div key={i} className="grid gap-2.5">
                    ...
                    {/* "How we know this" — the badge now opens its own working.
                        It appears only when every figure in the reply traced back
                        to this turn, and the drawer names each one: read from the
                        live data, or calculated from figures that were. A claim
                        of verification you cannot inspect is just a claim. */}
                    {m.verified && <EvidenceDrawer figures={m.evidence} t={t} />}
                    {m.blocks.map((b, j) => {
...
/**
 * The evidence drawer. Collapsed it is the same one-line assurance as before;
 * opened it shows every figure and where it came from, including the formula
 * for anything calculated. Nothing here is generated by the model — it is
 * computed from the reply and the turn's real data by lib/freehold/evidence.ts.
 */
function EvidenceDrawer({
  figures, t,
}: { figures?: { value: string; status: string; formula?: string }[]; t: (k: string) => string }) {
  const [open, setOpen] = useState(false)
  const traced = figures?.filter((f) => f.status !== 'ungrounded') ?? []

  if (traced.length === 0) {
    // Verified with nothing to itemise — say the sentence, offer no drawer.
    return (
  ...
  return (
    <div className="text-[10px]">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1.5 text-emerald-400/90 transition hover:text-emerald-300"
        aria-expanded={open}
      >
        <span aria-hidden>✓</span>
        {t('expert.verifiedNote')}
        <span className="text-emerald-400/60">{open ? '−' : '+'}</span>
      </button>

      {open && (
        <ul className="mt-1.5 space-y-1 rounded-lg border border-emerald-400/15 bg-emerald-400/[0.04] px-2.5 py-2">
          {traced.map((f, i) => (
            <li key={`${f.value}-${i}`} className="flex items-baseline gap-2">
              <span className="font-medium tabular-nums text-slate-200">{f.value}</span>
              <span className="text-slate-500">
                {f.status === 'grounded'
                  ? t('expert.evidence.fromData')
                  : `${t('expert.evidence.calculated')}${f.formula ? ` — ${f.formula}` : ''}`}
              </span>
            </li>
          ))}
        </ul>
