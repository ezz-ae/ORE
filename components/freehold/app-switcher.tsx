  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <div className="grid h-11 w-11 place-items-center rounded-2xl border border-white/10 bg-white/[0.04] backdrop-blur">
          <Icon className="h-5 w-5 text-white/90" />
        </div>
      </div>
      <div className="relative mt-10">
        <div className="text-base font-semibold tracking-tight text-white">{entry.label}</div>
        <div className="mt-1 text-[13px] leading-snug text-white/55">{entry.blurb}</div>
      </div>
    </Link>
  )
}
...
export function AppSwitcher({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[110] overflow-y-auto bg-[#06080A]/85 backdrop-blur-2xl">
      <div className="mx-auto max-w-5xl px-6 pb-24 pt-16 sm:pt-20">
        <div className="flex items-start justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.22em] text-[#D4AF37]/90">
          ...
          <button
            onClick={onClose}
            aria-label="Close"
            className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-white/10 bg-white/[0.04] text-white/70 transition hover:border-white/25 hover:bg-white/10 hover:text-white"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
