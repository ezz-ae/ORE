...
const QUICK_ACTIONS = [
  { label: "Freehold hot leads", prompt: "Show me Freehold's hottest leads today and explain the priority order", icon: Flame },
  { label: "Freehold ROI shortlist", prompt: "Build an Freehold shortlist of projects in inventory with 8% or more expected ROI", icon: TrendingUp },
  { label: "Freehold WhatsApp", prompt: "Draft an Freehold WhatsApp follow-up for a lead who viewed a Dubai Marina project", icon: MessageSquare },
  { label: "Freehold unassigned", prompt: "Show me all unassigned Freehold leads from the last 7 days", icon: Users },
]
    ...
    <Card className="rounded-[2rem] border-primary/20 bg-primary/5 shadow-sm overflow-hidden">
      <CardHeader className="flex flex-row items-center justify-between pb-3">
        <div className="space-y-0.5">
          <CardTitle className="font-serif text-2xl font-bold text-primary">Freehold Broker AI</CardTitle>
          <p className="text-[10px] uppercase font-bold tracking-widest text-primary/50">
            Freehold Sales Copilot
          </p>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        >
          {!hasMessages ? (
            <div className="py-4 text-center text-sm text-muted-foreground">
              Use the quick actions above or ask Freehold AI for the next best buyer move.
            </div>
          ) : (
          ...
          {isLoading && (
            <div className="flex items-center gap-2 px-2 py-1 text-xs text-muted-foreground">
              ...
              <span>Freehold AI is thinking...</span>
            </div>
          )}
          {/* Scroll anchor */}
          <div aria-hidden />
        ...
        <ChatInput
          onSend={sendMessage}
          disabled={isLoading}
          placeholder="Ask Freehold AI who to call, what to send, or which project fits..."
        />
      </CardContent>
    </Card>
  )
