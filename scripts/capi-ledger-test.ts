...
console.log('\n── the CRM event carries what a CRM event needs ──')
{
  const ev = buildQualifiedLeadEvent({
    eventId: 'fh-qualified-l1', stage: 'qualified', externalId: 'l1', leadId: 'meta-lead-9',
  }) as Record<string, unknown>
  const custom = ev.custom_data as Record<string, unknown>
  const user = ev.user_data as Record<string, unknown>

  // These events go to a CRM DATASET, not a website pixel — that is the
  // integration Conversion Leads optimisation reads. Meta pairs
  // action_source 'system_generated' with lead_event_source naming the system
  // reporting the outcome; without it an event carrying a lead_id is not
  // attached to the CRM integration and can be accepted while counting toward
  // nothing. Which fits the account: every ad set on Conversion Leads, and
  // three qualified leads across five weeks.
  // Meta's Qualified Leads guide, verbatim: lead_event_source is "the name of
  // your CRM" and event_source "should always be 'crm'". Both were absent.
  check('the outcome says which CRM reported it',
    typeof custom.lead_event_source === 'string' && (custom.lead_event_source as string).length > 0,
    JSON.stringify(custom))
  check('…and declares itself a CRM event, which is what attaches it',
    custom.event_source === 'crm', String(custom.event_source))

  // A 17-digit lead_id does not survive Number: MAX_SAFE_INTEGER is 16
  // digits, so the guide's unquoted example would round it to something that
  // matches nothing, with no error attached.
  check('the lead id is a string, so a 17-digit id cannot be rounded away',
    typeof user.lead_id === 'string')
  check('…and is marked as generated by a system, not a browser',
    ev.action_source === 'system_generated', String(ev.action_source))
  check('…and carries Meta\'s own lead id, unhashed',
    user.lead_id === 'meta-lead-9', String(user.lead_id))

  // The privacy line, restated here because this file now builds events.
  check('the person is never sent in the clear',
    !JSON.stringify(ev).includes('l1@') && Array.isArray(user.external_id))

  // No value is invented for a qualified lead — only a closed deal carries one.
  check('a qualified event carries no invented value',
    !('value' in custom), JSON.stringify(custom))
}
...
console.log('\n── and the sender actually records it ──')
{
  ...
  // THE CRM DATASET IS NOT THE WEBSITE PIXEL. Lead outcomes go to a CRM
  // dataset (Events Manager → CRM implementation); page views go to a browser
  // pixel. On this account the discovered pixel is named "test" and has never
  // fired, while the CRM dataset is a different id entirely — an outcome sent
  // to the pixel lands where no ad set optimises against it, which looks
  // exactly like success.
  check('the CRM dataset is its own destination, chosen first',
    /META_CRM_DATASET_ID/.test(capi) && /source: 'crm'/.test(capi),
    'outcomes would go to the browser pixel')
  check('…and it can be set without a redeploy',
    /crmDatasetId/.test(
      readFileSync(join(process.cwd(), 'app/api/freehold/integrations/meta/credentials/route.ts'), 'utf8')))
