...
async function debug() {
  try {
    ...
    const projectsCount = await query("SELECT count(*) FROM freehold_site_projects");
    console.log("Total projects in freehold_site_projects:", projectsCount[0].count);

    const statusCount = await query("SELECT status, count(*) FROM freehold_site_projects GROUP BY status");
    console.log("Status distribution:", statusCount);

    const areasCount = await query("SELECT count(*) FROM freehold_site_area_profiles");
    console.log("Total area profiles:", areasCount[0].count);

    const developersCount = await query("SELECT count(*) FROM freehold_site_developer_profiles");
