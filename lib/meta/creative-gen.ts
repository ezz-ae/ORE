/**
 * Ad-copy generation engine — extracted verbatim from
 * app/api/meta/creatives/generate/route.ts so server-side callers (the Ads
 * Machine planner, agent tools) can generate grounded creative without going
 * through an HTTP round-trip. The route now imports from here.
 *
 * Two paths, both grounded on REAL listing fields:
 *  - geminiCreatives: live Gemini generation; returns null on no-key/error/bad
 *    output so callers fall back honestly.
 *  - buildVariants: the deterministic template fallback — every line is built
...
{"variants":[{"primaryText":"...","headline":"...","description":"..."}]}`
  try {
    let data
    try {
      data = await geminiGenerate(apiKey, [{ role: 'user', parts: [{ text: prompt }] }], { temperature: 0.85, maxOutputTokens: 2048, responseMimeType: 'application/json' })
    } catch { return null }
