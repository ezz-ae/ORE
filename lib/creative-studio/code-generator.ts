  ...
  const generateNodeCode = (nodeId: string, indent = "  "): string => {
    ...
    switch (node.type) {
      ...
      case "tool":
        ...
        const toolCode = String(node.data.code || "return { result: 'Tool executed' };")
      ...
      case "javascript":
        ...
        const jsCode = String(node.data.code || "return input1;")
      ...
      case "videoGeneration": {
        nodeCode += `${indent}// Video Generation Node (fal.ai Veo 3.1)\n`
...
export function generateRouteHandlerCode(nodes: Node[], edges: Edge[]): string {
  ...
  if (hasVideoGen) {
    code += `    // Video Generation (fal.ai Veo 3.1)\n`
